...
az deployment group create \
  --name audiochoice-staging-analysis-2p2-api \
  --resource-group "$resource_group" \
...
az deployment group create \
  --name audiochoice-staging-analysis-2p2-worker \
  --resource-group "$resource_group" \
...
echo
echo 'Analysis 2.2 scene-detection deployment completed.'
