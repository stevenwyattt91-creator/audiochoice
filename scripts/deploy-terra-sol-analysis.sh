...
az deployment group create --name audiochoice-staging-analysis-3p1-api \
...
az deployment group create --name audiochoice-staging-analysis-3p1-worker \
...
az containerapp show --name audiochoice-stg-worker --resource-group "$resource_group" \
  --query "properties.template.containers[0].env[?contains(name, 'Model') || contains(name, 'ScannerVersion') || contains(name, 'MaximumScene')].{Name:name,Value:value}" \
  --output table

echo 'Cost-balanced Luna discovery, Terra verification, and capped Sol escalation deployment completed.'
