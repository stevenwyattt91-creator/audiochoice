...
az deployment group create \
  --name audiochoice-staging-synced-book-filters-api \
  --resource-group "$resource_group" \
...
az deployment group create \
  --name audiochoice-staging-synced-book-filters-worker \
  --resource-group "$resource_group" \
...
echo
echo 'Synced per-audiobook filters and taxonomy 2.0 deployment completed.'
