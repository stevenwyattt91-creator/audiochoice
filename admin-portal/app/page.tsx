...
type AffiliateSummary = { id: string; code: string; label: string; email?: string; active: boolean; createdAt: string; referralCount: number };
type AffiliateReferral = { userID: string; email: string; displayName: string; attributedAt: string };
type AffiliateDetail = { affiliate: AffiliateSummary; referrals: AffiliateReferral[] };
...
export default function Home() {
  ...
  const [catalog, setCatalog] = useState<CatalogBook[]>([]); const [audits, setAudits] = useState<Audit[]>([]); const [payments, setPayments] = useState<Payment[]>([]); const [affiliates, setAffiliates] = useState<AffiliateSummary[]>([]); const [selected, setSelected] = useState<CatalogBook | null>(null); const [detail, setDetail] = useState<any>(null); const [search, setSearch] = useState(""); const [users, setUsers] = useState<Access[]>([]); const [busy, setBusy] = useState(false); const [setup, setSetup] = useState(false);
  ...
  const load = async (supplied = token) => { const me = await call("/v1/internal/me", {}, supplied) as Access; if (me.role !== "admin") throw new Error("This account is not approved as an AudioChoice administrator."); setUser(me); const [summary, books, taskList, paymentList, people, affiliateList] = await Promise.all([call("/v1/internal/admin/dashboard", {}, supplied), call("/v1/internal/admin/catalog", {}, supplied), call("/v1/internal/audits", {}, supplied), call("/v1/internal/admin/payments", {}, supplied), call("/v1/internal/admin/users", {}, supplied), call("/v1/internal/admin/affiliates", {}, supplied)]); setDashboard(summary); setCatalog(books); setAudits(taskList); setPayments(paymentList); setUsers(people); setAffiliates(affiliateList); };
...
function Affiliates({ items, call, refresh, setError }: any) {
  const [adding, setAdding] = useState(false);
  const [busy, setBusy] = useState(false);
  const [opened, setOpened] = useState<AffiliateDetail | null>(null);
  const create = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    const form = new FormData(e.currentTarget);
    setBusy(true);
    try {
      await call("/v1/internal/admin/affiliates", { method: "POST", body: JSON.stringify({ label: form.get("label"), email: form.get("email") || null }) });
      setAdding(false);
      await refresh();
    } catch (err) { setError((err as Error).message); } finally { setBusy(false); }
  };
  const toggleActive = async (affiliate: AffiliateSummary) => {
    try { await call(`/v1/internal/admin/affiliates/${affiliate.id}/active`, { method: "PUT", body: JSON.stringify({ active: !affiliate.active }) }); await refresh(); }
    catch (err) { setError((err as Error).message); }
  };
  const openDetail = async (affiliate: AffiliateSummary) => {
    try { setOpened(await call(`/v1/internal/admin/affiliates/${affiliate.id}`)); }
    catch (err) { setError((err as Error).message); }
  };
  return <>
    <div className="panelhead">
      <div><p className="eyebrow">AFFILIATE PROGRAM</p><h2>Referral codes and attribution</h2><p className="muted">Each affiliate gets a short code. An account that signs up with it is attributed here immediately -- no subscription or payment step is required for that part.</p></div>
      <button onClick={() => setAdding(!adding)}>{adding ? "Cancel" : "Create affiliate"}</button>
    </div>
    {adding && <form className="create" onSubmit={create}>
      <label>Affiliate name<input name="label" required placeholder="e.g. Caroline @ No Spice Books" /></label>
      <label>Email (optional)<input name="email" type="email" placeholder="affiliate@example.com" /></label>
      <button disabled={busy}>{busy ? "Creating…" : "Generate code"}</button>
    </form>}
    <section className="panel">
      <div className="table">
        {items.map((a: AffiliateSummary) => <div className="row task" key={a.id}>
          <span><strong>{a.label}</strong><small>{a.email || "No email on file"}</small></span>
          <span><b className="pill">{a.code}</b><small>created {formatDate(a.createdAt)}</small></span>
          <span>{a.referralCount} referred account{a.referralCount === 1 ? "" : "s"}</span>
          <span className="actions">
            <button className="ghost" onClick={() => openDetail(a)}>View referrals</button>
            <button className="ghost" onClick={() => toggleActive(a)}>{a.active ? "Deactivate" : "Reactivate"}</button>
          </span>
        </div>)}
        {!items.length && <p className="empty">No affiliates yet. Create one to get a referral code.</p>}
      </div>
    </section>
    <section className="notice">
      <strong>Discounted first month</strong>
      <p>A referred user's discount (e.g. $2.99 for the first month instead of $5.99) is applied through Apple's and Google's own subscription offer systems once in-app purchase is live, not through this list. This page only tracks which affiliate gets credit for which signup.</p>
    </section>
    {opened && <div className="overlay" onMouseDown={() => setOpened(null)}>
      <section className="drawer" onMouseDown={e => e.stopPropagation()}>
        ...
        <h2>{opened.affiliate.label}</h2>
        <p>Code <b className="pill">{opened.affiliate.code}</b> · {opened.affiliate.email || "No email on file"}</p>
        <div className="detailstats"><span><b>{opened.referrals.length}</b> referred accounts</span></div>
        <section>
          <h3>Referred accounts</h3>
          {opened.referrals.length
            ? <div className="table">{opened.referrals.map(r => <div className="row task" key={r.userID}><span><strong>{r.displayName}</strong><small>{r.email}</small></span><span>{formatDate(r.attributedAt)}</span><span /></div>)}</div>
            : <p className="muted">No one has signed up with this code yet.</p>}
        </section>
      </section>
    </div>}
  </>;
